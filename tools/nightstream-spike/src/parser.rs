//! Self-contained Circom .r1cs + .wtns binary parsers. Re-implementation of the
//! parser from tools/r1cs-latticefold/src/lib.rs — duplicated rather than shared
//! via a path-dep because that crate transitively pulls in LatticeFold +
//! stark-rings, which require nightly Rust and are unrelated to Nightstream.
//!
//! Format specs:
...
pub fn parse_circom_r1cs(path: &Path) -> Result<CircomR1cs> {
    ...
    let mut header: Option<(u32, u32, u32, u32, u32)> = None; // (fsb, nw, npo, npi, nc)
    ...
    let (fsb, nw, npo, npi, nc) =
        header.ok_or_else(|| anyhow!("missing header"))?;
